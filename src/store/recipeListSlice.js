import { createSlice } from "@reduxjs/toolkit";
import storage from "../Storage";

const initialState = [
//   {
//     id: 0,
//     name: "Basic Template",
//     image: "",
//     count: 0,
//     ingredients: [
//       {
//         name: "Ingredient 1",
//         unit: "Cup",
//         quantityUnit: "1",
//       },
//       {
//         name: "Ingredient 2",
//         unit: "Tbsp",
//         quantityUnit: "1",
//       },
//     ],
//     time: {
//       prep: 20,
//       active: 40
//     },
//     difficulty: "Easy",
//     calories: 370,
//     favorite: false,
//     dishesAmount: "Small (1-4)",
//     yield: 0,
//     directions: [
//       "",
//       ""
//     ]
//   },
//   {
//     id: 1,
//     name: "One-Pan Pork Tenderloin with Green Beans and Potatoes",
//     image: "",
//     count: 0,
//     ingredients: [
//       {
//         name: "Unsalted Butter (Softened)",
//         unit: "Tbsp",
//         quantityUnit: "4",
//       },
//       {
//         name: "Minced Fresh Chives",
//         unit: "Tbsp",
//         quantityUnit: "2",
//       },
//       {
//         name: "Minced Garlic",
//         unit: "Clove",
//         quantityUnit: "1",
//       },
//       {
//         name: "Salt and Pepper",
//         unit: "",
//         quantityUnit: "",
//       },
//       {
//         name: "Green Beans (Trimmed)",
//         unit: "Lbs",
//         quantityUnit: "1",
//       },
//       {
//         name: "Extra Virgin Olive Oil",
//         unit: "Tbsp",
//         quantityUnit: "2",
//       },
//       {
//         name: "Small Red Potatoes (Peeled, Halved)",
//         unit: "Lbs",
//         quantityUnit: "1.5",
//       },
//       {
//         name: "Pork Tenderloin (Trimmed)",
//         unit: "Whole",
//         quantityUnit: "1-2",
//       },
//       {
//         name: "Hoisin Sauce",
//         unit: "Tbsp",
//         quantityUnit: "2",
//       },
//     ],
//     time: {
//       prep: 0,
//       active: 0
//     },
//     difficulty: "",
//     calories: 0,
//     favorite: false,
//     dishesAmount: "",
//     yield: 0,
//     directions: [
//       "Adjust oven rack to lower-middle position and heat oven to 450 degrees. \
// Combine butter, chives, garlic, 1/4 teaspoon salt, and 1/4 teaspoon pepper in bowl; set aside",
//       "Toss green beans with 1 tablespoon oil, 1/4 teaspoon salt, and....",
//       "Pat pork dry with paper towels and season with pepper. Brush pork all over with hoisin sauce. \
// Lay pork lengthwise...",
//       "Gently stir vegetables on sheet to combine. Return sheet to oven....",
//     ]
//   },
//   {
//     id: 2,
//     name: "Best Beef Stew",
//     image: "",
//     count: 0,
//     ingredients: [
      
//       {
//         name: "Bonless Beef Chuck-eye Roast (Pulled apart at seams, trimmed, cut in to 1-1.5 inch pieces)",
//         unit: "Lbs",
//         quantityUnit: "4",
//       },
//       {
//         name: "Salt and Pepper",
//         unit: "",
//         quantityUnit: "",
//       },
//       {
//         name: "Canola Oil",
//         unit: "Tbsp",
//         quantityUnit: "3",
//       },
//       {
//         name: "Unsalted Butter",
//         unit: "Tbsp",
//         quantityUnit: "2",
//       },
//       {
//         name: "Onions (Chopped)",
//         unit: "",
//         quantityUnit: "2",
//       },
//       {
//         name: "Garlic (Minced)",
//         unit: "Clove",
//         quantityUnit: "3",
//       },
//       {
//         name: "All-Purpose Flour",
//         unit: "Cup",
//         quantityUnit: "1/4",
//       },
//       {
//         name: "Tomato Paste",
//         unit: "Tbsp",
//         quantityUnit: "1",
//       },
//       {
//         name: "Dry Red Wine",
//         unit: "Cup",
//         quantityUnit: "1",
//       },
//       {
//         name: "Beef Broth",
//         unit: "Cup",
//         quantityUnit: "3.5",
//       },
//       {
//         name: "Carrots (Peeled and Sliced 1 Inch Thick)",
//         unit: "Lbs",
//         quantityUnit: "1",
//       },
//       {
//         name: "Bay Leaf",
//         unit: "Leaf",
//         quantityUnit: "2",
//       },
//       {
//         name: "Fresh Thyme (Minced)",
//         unit: "Tbsp",
//         quantityUnit: "1",
//       },
//       {
//         name: "Red Potatoes (Unpeeled, Cut Into 1 Inch Pieces)",
//         unit: "Lbs",
//         quantityUnit: "1.5",
//       },
//       {
//         name: "Frozen Peas",
//         unit: "Cup",
//         quantityUnit: "1",
//       },
//       {
//         name: "Fresh Parsley (Minced)",
//         unit: "Tbsp",
//         quantityUnit: "3",
//       },
//     ],
//     time: {
//       prep: 0,
//       active: 0
//     },
//     difficulty: "",
//     calories: 0,
//     favorite: false,
//     dishesAmount: "",
//     yield: 0,
//     directions: [
//       "Adjust oven rack to lower-middle position and heat oven to 325 degrees. \
// Pat beef dry with paper towels and season lightly with salt and pepper. Heat 2 tablesspoons oil in Duth oven over medium-high heat until just smoking. Add half of meat and cook, stirring occasionally, until well browned, 7 to 10 minutes, reducing heat if pot begins to scorch; transfer to bowl. Repeat with remaining 1 tablespoon oil and remaining beef.",
//       "Melt butter in now-empty pot over medium-low heat. Add onions and 1/2 teaspoon salt and cook, stirring often, until softened, 5 to 7 minutes. Stir in garlic and cook until fragrant, about 30 seconds. Stir in flour and tomato paste and cook, stirring constantly, until golden, about 1 minute. Slowly whisk in wine, scraping up any browned bits. Gradually whisk in broth until smooth and bring to simmer.",
//       "Stir in browned meat, carrots bay leaves and thyme and bring to simmer. Cover, place pot in oven, and cook for 1 hour. Stir in potatoes and continue to cook in oven, covered, until meat is tender, 1.5 to 2 hours longer. Remove pot from oven and discard bay leaves.",
//       "Season with salt and pepper to taste. Stir in peas, cover, and let sit for 10 minutes. Serve, sprinkling parsley over individual portions.",
//     ]
//   },

]
// console.log(initialState);

const recipeListSlice = createSlice({
  name: "recipeList",
  initialState: initialState,
  reducers: {
    loadRecipeList: async (state, action) => {
      await storage.load({key: "recipeList"}).then((ret) => {
        state.value = ret
      })
    },
    saveRecipeList: (state, action) => {
      console.log(state);
      storage.save({
        key: "recipeList",
        data: state,
        expires: null
      })
    },
    addRecipe: (state, action) => {
      state.push(action.payload)
    },
    deleteRecipe: (state, action) => {
      state.value.splice(action.payload, 1)
    }
  }
})

export const { addRecipe, deleteRecipe, loadRecipeList, saveRecipeList } = recipeListSlice.actions;

export default recipeListSlice.reducer;